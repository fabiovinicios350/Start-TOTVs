...
Function Main()
   ...
   //Declarando Variaveis
    ...
    PreencherMatriz(@aMatrizB,"B",3,5)

    for i:=1 to len(aMatrizA)
        aVector := {}
        for j:= 1 to len(aMatrizA[i])
            AAdd(aVector,aMatrizA[i][j])
        next j
        AAdd(aMatrizC,aVector)
    next i

    for i:=1 to len(aMatrizB)
        aVector := {}
        for j:= 1 to len(aMatrizB[i])
            AAdd(aVector,aMatrizB[i][j])
        next j
        AAdd(aMatrizC,aVector)
    next i
